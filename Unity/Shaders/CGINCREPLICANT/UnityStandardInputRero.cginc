...
{
    float4 texcoord;
    texcoord.xy = TRANSFORM_TEX(((_UVPrimary == 0) ? v.uv0 : v.uv1), _MainTex); // Always source from uv0
...
half Occlusion(float2 uv)
{
    ...
    if(_occlusionChannel == 0)
        occ = tex2D(_MetallicGlossMap, uv).r;
    else if(_occlusionChannel == 1)
        occ = tex2D(_MetallicGlossMap, uv).g;
    else if(_occlusionChannel == 2)
        occ = tex2D(_MetallicGlossMap, uv).b;
    else
        occ = tex2D(_MetallicGlossMap, uv).a;
...
half2 MetallicGloss(float2 uv)
{
    ...
    #endif
    if (_glossChannel == 0)
        mg.g = 1.0f - tex2D(_MetallicGlossMap, uv).r;
    else if (_glossChannel == 1)
        mg.g = 1.0f - tex2D(_MetallicGlossMap, uv).g;
    else if (_glossChannel == 2)
        mg.g = 1.0f - tex2D(_MetallicGlossMap, uv).b;
    else
        mg.g = 1.0f - tex2D(_MetallicGlossMap, uv).a;
    mg.g *= _GlossMapScale;
#else
    mg.r = _Metallic;
    #ifdef _SMOOTHNESS_TEXTURE_ALBEDO_CHANNEL_A
        mg.g = 1.0f - tex2D(_MainTex, uv).a * _GlossMapScale;
    #else
        mg.g = _Glossiness;
    #endif
#endif
